...
def etf_detail():
    ...
    try:
        ...
        df = db.get_close_by_isin(identifier, days=2000)
        ...
        if not df.empty:
            ...
            df_old = db.get_ohlcv(_isin or ticker, days=2000)
            if not df_old.empty:
        ...
        if not df.empty and 'close' in df.columns:
            ...
            # La modal ha i bottoni 30/120gg/1anno/Max che affettano questa serie
            # lato client (history.slice(-days)): va inviata l'intera serie
            # disponibile (con il backfill Golden Dataset ~4,5 anni) per il tasto Max.
            for _, row in df.tail(1600).iterrows():
                price_hist.append({
...
@app.route('/api/bounce-radar')
def get_bounce_radar():
    """
    Radar Rimbalzo EMA20 (2026-08-25, idea utente): ETF gia' sopra la EMA20
    che stanno ritracciando verso di essa ma, prima di romperla, invertono la
